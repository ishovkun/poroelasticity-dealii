...
namespace poro_elastisity {
  ...
  public:
    PoroElasticProblem(input_data::InputDataPoroel &data_);
    ~PoroElasticProblem();
    void run ();
  ...
  template <int dim>
  PoroElasticProblem<dim>::PoroElasticProblem(input_data::InputDataPoroel &data_) :
    data(data_),
    pressure_solver(triangulation, data_),
    displacement_solver(triangulation, data_)
  {
  ...
  void PoroElasticProblem<dim>::run()
  {
    ...
    std::cout << "time max " << data.t_max  << std::endl;
    while (time < data.t_max){
      ...
      if (time_step_number % 5 == 0){
        ...
        refine_mesh(data.initial_refinement_level,
                    data.initial_refinement_level +
                    data.max_refinement_level);
        displacement_solver.assemble_system(pressure_solver.solution);
        strain_projector.assemble_projection_matrix();
      }
  ...
  template <int dim>
  void PoroElasticProblem<dim>::create_mesh ()
  {
    Tensor<1, dim> point_1, point_2;

    for(int i=0; i<dim; ++i){
      point_1[i] += data.domain_size[i]/2;
      point_2[i] -= data.domain_size[i]/2;
    }
...

  {
    ...
    std::vector< Vector<double> > previous_solution(3);
    previous_solution[0] = pressure_solver.solution;
    previous_solution[1] = volumetric_strain;
    previous_solution[2] = initial_volumetric_strain;
